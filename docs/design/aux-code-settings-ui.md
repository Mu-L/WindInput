...
7. 照 `dialogs/schema_codetable.rs` 的既定模式做「方案级辅助码配置」：**每项一个勾选框，
   勾上才写这一项，没勾就是 `null`（跟随）；没勾的行显示跟随值，只读**。落
   `schema_overrides/{id}.toml`，并按既有规矩登记 `SideCommitter`（非 `config.toml` 落点
   一律登记，禁止另加保存按钮）。

   > ⚠️ **本条在 0.119 反转过。** 原文写的是「一个总开关，关＝整体跟随、开＝整套自己配
   > （不是每字段一个 tri-state 控件——那个模式本仓已经权衡过并否决）」。整段式总开关
   > 实测有三个毛病：改一项就让整个方案不再跟随全局；方案作者写死的值也被算进「已自定义」
   > 于是界面说谎；用户看不出某项的值是谁定的。三态与 core 的 `Option<T>` 同构，一次治掉
   > 三条。**照抄前先读 `schema_codetable.rs` 的头部注释**，那里有完整论证与三份数据
   > （生效值 / 跟随值 / override 层）的分工。
