...
> 开关为 `schema.codetable.short_code_yield_level`（0 关闭 / 2 一二级 / 3 全部）。
> **出厂全关**：全局基线 0，且内置方案（含 wubi86）一律不声明这一项，理由见 §6.3。
> 词库层的 `gen_dict` `[demotion]` 已同步退役，发行词库保留极点上游的候选次序。
...
### 6.3 档位的出厂值：出厂全关，一个字都不替用户做主

分两步走到今天，两步的理由不同，都要记住。

**第一步（0.119 上午）：全局 3 → 全局 0。** `schema.codetable` 是**所有码表方案的公共
基线**，而 §2 那条判据——「简码是全码的前缀，故短码首选就是简码字」——只对五笔这类
**前缀式简码体系**成立。第三方码表未必如此：词频码表的短码首选是作者按频次排定的常用字，
二三重码表的短码另有编码规则，它们的短码首选并不是「同一个字的简码」。对这些方案让位，
沉底的不是「用户已经能用更短的码打出来」的字，而是作者精心排在前面的常用字——纯属破坏，
且用户根本不知道有这么一项在起作用。

**第二步（同日，撤回 wubi86 的方案级 3）。** 第一步顺手给内置 wubi86 的
`[engine.codetable]` 补了 `short_code_yield_level = 3`，让五笔用户手感延续。当天撤回，
判据是**用户在全局页做的事必须能作用到出厂方案上**：

方案级的 `Some(_)` 恒覆盖全局。于是五笔用户打开全局页，把「出简让全」调成任何档位，
候选顺序都纹丝不动——而他不会想到去方案级码表配置里找那个标着「方案自带」的小标记。
从他的角度这就是「设置项坏了」。内置方案给自己配特例，等于把全局页那一项对它变成摆设，
这个代价大于让位功能本身。

⇒ **出厂全关，内置方案一律不声明这一项。** 想要的人自己开，全局页或方案级都行。

这一条对第三方方案作者仍然成立：他们的 `[engine.codetable]` 里写 `short_code_yield_level`
是完全正当的——那是作者对自己码表的判断，且用户装这张码表时就接受了作者的安排。
区别在于**内置方案不该有特权**：用户没有「选择安装 wubi86」这个动作，它是出厂就在那儿的。

配套地，**词库侧也不代做**。`gen_dict` 的 `[demotion]`（词库生成阶段的简码降权）在本功能
落地时就已退役（§1），词库重新生成后产物已完整回到极点上游的候选次序——`khtk` 是
「路」2383 >「路程」1435，`dddd` 是「大」8030 >「大厦」8020（后者由 `[protected_codes]`
按上游优先级发权重）。运行时关、词库层也不动，出厂状态下用户拿到的就是**词库作者排的
那个顺序**。

用户侧的可见性没有损失：全局页那一项还在（出厂关），方案级码表配置里那一行也还在，
只是 wubi86 上它现在显示「跟随全局」而不是「方案自带」。

升级路径（`set_user_value` 的语义是「等于当前默认就删键」，故分两批）：

- 0.118 期间**没动过**这一项、或把它设成 3 的用户 —— 用户层没有这个键（设成 3 时等于当时
  的默认，写入当场被删）⇒ 升级后跟随新默认 0，**候选顺序会变**。这对第三方码表用户正是
  本次改动的目的；对五笔用户则是「功能被关掉了」，两者都须写进 changelog，并告诉五笔
  用户在哪儿开回来。
- 主动设成 0 或 2 的用户 —— 显式值与当时的默认不同，保留，不受影响。
...
⚠️ 这条取舍的**影响面随 §6.3 的出厂全关而缩小**：出厂状态下 `dddd` 首选就是「大」，
键位约定完好；只有用户自己开了这个功能才会看到「大厦」。所以 §6.2 现在描述的是
「开了之后是什么样」，而不是「用户拿到手是什么样」——排查「键名汉字码首选不对」时，
第一步是问他有没有开出简让全，而不是去查 `[protected_codes]`。
