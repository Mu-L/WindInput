...
> **全局出厂 0（关），内置 wubi86 在自己的方案文件里声明 3**，理由见 §6.3。
...
### 6.3 档位的出厂值：全局关、由方案声明

0.118 出厂把**全局**开到 3，理由是与它取代的 `[demotion]` 同量级（239 vs 205 个码），
升级后手感延续。0.119 改判：

`schema.codetable` 是**所有码表方案的公共基线**，而 §2 那条判据——「简码是全码的前缀，
故短码首选就是简码字」——只对五笔这类**前缀式简码体系**成立。第三方码表未必如此：
词频码表的短码首选是作者按频次排定的常用字，二三重码表的短码另有编码规则，它们的短码
首选并不是「同一个字的简码」。对这些方案让位，沉底的不是「用户已经能用更短的码打出来」
的字，而是作者精心排在前面的常用字——纯属破坏，且用户根本不知道有这么一项在起作用。

故出厂值改为：**全局 0，适合的方案在自己的 `[engine.codetable]` 里声明**。这与
`z_key_action` 是同一个模式——「这张码表适不适合」只有方案作者知道。内置 wubi86 写的是
`short_code_yield_level = 3`（混输方案 `wubi86_pinyin` 经 `primary_schema` 自动继承）。

用户侧不受影响：设置页的方案级码表配置里这一项照常可见可改——那一行不勾选时显示的就是方案文件里的 3，且来源标着「方案自带」，
全局页那一项也还在，只是出厂关。

升级路径（`set_user_value` 的语义是「等于当前默认就删键」，故分两批）：

- 0.118 期间**没动过**这一项、或把它设成 3 的用户 —— 用户层没有这个键（设成 3 时等于当时
  的默认，写入当场被删）⇒ 升级后跟随新默认 0。五笔用户由方案文件补回 3，无感；用别的
  码表方案的用户会感到候选顺序变了，**这正是本次改动的目的**，须写进 changelog。
