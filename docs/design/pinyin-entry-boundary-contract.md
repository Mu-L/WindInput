...
| 落点 | 现状 | 判定 |
|---|---|---|
| `user_words.rs:252 on_word_selected` | **硬编码 0**，注释写「此处只有扁平 code，无边界可算 → 0」，且是**隐性造词路径**（不存在则凭空创建 weight=0 记录） | ✗ 违反 |
| `webdata/lib.rs:553 infer_boundary_for` | 推导码 != 手输码 → 0 | ✗ 违反（可用第 4 层补上） |
...
| `on_word_selected` 隐性造词、自动造词（无 UI、用户正在打字） | **`debug_assert` + WARN，照写** |
...
更强的守门（可选）：把 `boundary` 参数换成不可默认构造的类型，强制每个调用点交代来源。
P2a 用过这招，「这一逼问才发现 `on_word_selected` 这条此前无人知道的隐性造词路径」。

## 7. 存量数据

不变量对已入库的数据不成立，需走同一条求解链回填一次。

★ 建议做成词库管理里的**显式动作**而非静默迁移：补上边界会改变这些词的简拼行为
（`abbrev_index` 的键要从 `\u{1}n` 兜底组 `shift` 到真声母组，见
`user_words.rs:322` 里 `nb != b` 那段的现成处理），用户应当知道自己的词库行为变了。
