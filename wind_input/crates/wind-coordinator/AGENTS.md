...
- **辅助码与翻页共键**：`session_actions` 的 `aux_code:page_next` 是 `aux_code` 这个动词的**参数**（`AuxCodeShare`），不是新动词、不是通用降级链（后者已否决，见 `docs/design/key-resolver-unification.md`）。语义「顺序即优先级」：`apply_session_action` 先调 `enter_aux_code`，返回 `None`（已在别的 overlay / 未启用 / 无码表 / 无候选）才降级为 `NavAction::PageNext`，**落到同一段 nav 执行**——别在那条臂里自己写 `page_next` + `notify_ui_update`。模式内翻页、未开启退化成纯翻页键，都是这条降级的副产品，不要为它们各加特判。★ `is_aux_code_trigger`（「再按一次退出」）**只认 `AuxCodeShare::Solo`**：共键若也算触发键，用户永远翻不到第二页。
